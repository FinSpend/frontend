...
import {
  ...
  X, Eye, EyeOff, Loader2, AlertCircle,
} from 'lucide-react';
import { useData } from '@/app/lib/data-context';
import { useToast } from '@/app/Components/ui/ToastProvider';
import { formatIDR } from '@/app/lib/format';
import { changePassword, deleteAccount } from '@/app/services/userService';
...
export function ProfilePage() {
  const { user, transactions, budgets, profile, clearAll } = useData();
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center gap-2 shrink-0">
              {!isPremium && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm transition-colors shadow-sm"
                >
              ...
              <Link
                href="/settings"
                className="flex items-center gap-1.5 px-3 py-1.5 border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl text-sm transition-colors"
              >
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={label} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-5 text-center">
            ...
            <p className={`text-3xl ${text}`}>{value}</p>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-6">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="text-lg dark:text-white">Skor Keuangan</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Berdasarkan riwayat transaksi Anda</p>
          </div>
          <div className="text-right">
            <span className={`text-5xl font-light ${scoreTextColor}`}>{financialScore}</span>
        ...
        <div className="flex items-center justify-between">
          <span className={`text-sm ${scoreTextColor}`}>{scoreLabel}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {profile && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-6">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg dark:text-white">Profil Keuangan</h3>
              <Link href="/settings" className="flex items-center gap-1 text-sm text-blue-600 dark:text-blue-400 hover:underline">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-6">
            ...
            {profile.financialGoals.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-32 text-center">
                ...
                <Link href="/settings" className="text-sm text-blue-600 dark:text-blue-400 hover:underline">Tambah tujuan →</Link>
              </div>
            ) : (
              <div className="flex flex-wrap gap-2">
                {profile.financialGoals.map((goal) => {
                  ...
                  return (
                    <span key={goal} className="flex items-center gap-2 px-4 py-2.5 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded-xl text-sm border border-blue-100 dark:border-blue-800">
                      {g.icon} {g.label}
                    </span>
                  );
                })}
      ...
      {/* ── Account Actions ── */}
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-6">
        <h3 className="text-lg dark:text-white mb-1">Keamanan Akun</h3>
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 text-sm transition-colors"
          >
            <KeyRound className="w-4 h-4" /> Ubah Password
          </button>
          <button
            onClick={() => { setDeleteEmail(''); setShowDelete(true); }}
            className="flex items-center gap-2 px-4 py-2.5 border border-red-200 dark:border-red-900 text-red-600 dark:text-red-400 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 text-sm transition-colors"
          >
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-sm">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base dark:text-white">Ubah Password</h3>
              </div>
            ...
            <form onSubmit={handleChangePassword} className="p-6 space-y-4">
              ...
              {(['current', 'next', 'confirm'] as const).map((key) => {
                ...
                return (
                  <div key={key}>
                    <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1.5">{labels[key]}</label>
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-2.5 border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 text-sm transition-colors"
                >
                  Batal
                </button>
                <button
                  type="submit"
                  disabled={pwLoading}
                  className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white rounded-xl text-sm transition-colors flex items-center justify-center gap-2"
                >
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-sm">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base dark:text-white">Hapus Akun</h3>
              </div>
            ...
            <form onSubmit={handleDeleteAccount} className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white rounded-xl text-sm transition-colors flex items-center justify-center gap-2"
                >
